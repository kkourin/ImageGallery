...
namespace ImageGallery
{
    public class IconHelpers
    {
...
#if DEBUG
            Console.WriteLine($"icon id: {sfi.iIcon} psz: {pszFile}");
#endif

            return sfi.iIcon;
        }

        public static byte[] GetExtraLargeIcon(string extension)
        {
            var trimmed = extension.TrimStart(new char[] { '.' });
            var iconIndex = GetIconIndex("*." + trimmed);
            if (iconIndex == 0)
            {
                throw new NullReferenceException("Got null for icon index.");
            }
            IntPtr hIcon = GetExtraLargeIconNative(iconIndex);
        ...
        // 256*256
        static IntPtr GetExtraLargeIconNative(int iImage)
        {
