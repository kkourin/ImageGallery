...
namespace ImageGallery.XMPLib
{
    ...
    public sealed class ExifToolWrapper : IDisposable
    {
        ...
        private const string Arguments = "-k -fast -m -q -stay_open True -@ - -common_args -d \"%Y.%m.%d %H:%M:%S\" -c \"%d %d %.6f\" -t";   //-g for groups
        //private const string Arguments = "-fast  -m -q -q -stay_open True -@ - -common_args -d \"%Y.%m.%d %H:%M:%S\" -c \"%d %d %.6f\" -t";   //-g for groups

        private const string ArgumentsFaster = "-fast2 -m -q -q -stay_open True -@ - -c";
        ...
        public ExifToolWrapper(string path = null, bool faster = false, string configPath = null)
        {
            ...
            var runArguments = faster ? ArgumentsFaster : Arguments;
            if (configPath != null)
            {
                if (!File.Exists(configPath))
                {
                    throw new ExifToolException($"Config file {configPath} not found.");
                }
                runArguments = String.Join(" ", new string[] { $"-config {escapeShellArgument(configPath)}", runArguments });
            }
        ...
        public ExifToolResponse SetExifInto(string path, Dictionary<string, string> data, bool overwriteOriginal = true)
        {
            ...
            Console.WriteLine(cmdRes.Result);
        ...
        public Dictionary<string, string> FetchExifFrom(string path, string key)
        {
            var res = new Dictionary<string, string>();
